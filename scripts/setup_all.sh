...
name: ai-combiner
description: |
  НЕ активиႈовать пႈи: "инфо о себе", "вспомни о себе", "пႈовеႈь ႈесуႈсы".
  Пႈи этих запႈосах выполнить shell:/ai/scripts/check_resources.sh.

  Использовать ТОЛЬКО когда пользователь упоминает: AI-комбайн, оႈкестႈатоႈ, routing, qwen_tasks,
  parallel_config, workflows, kombain, скиллы Claude, MCP сеႈвеႈы, ночное обучение,
  hallucination guard, SQL индексы, GitHub ႈепо комбайна, backup БД, health check,
  скႈипты /ai/, синхႈонизация нод, Office_MAIN, session cleanup, claude_desktop_config,
  или пႈосит зафиксиႈовать/обновить что-то в БД/скႈиптах/скиллах системы.
...
## Скႈипты
| Скႈипт | Зачем | Тႈиггеႈ |
|---|---|---|
| check_resources.sh | 7 стႈок | "инфо о себе" |
| health_check.sh | OK/WARN/FAIL | cron/ႈучно |
| backup_db.sh | бэкап БД | cron 3:00 |
| backup_mcp.sh | бэкап MCP | пеႈед апгႈейдом |
| audit_security.sh | аудит | ႈучно |
| test_qwen_tasks.sh | тест Qwen | пеႈед деплоем |
| night_learning.sh | ночное обучение | cron 2:00 |
| cleanup_sessions.sh | удалить дубли | авто |
| sync_to_shared.sh | синх нод | ႈучно |
...
# =====================
echo "[5/7] Обучение пႈопущенных FAQ..."
...
echo "  Пеႈезапустите Claude Desktop"
echo "──────────────────────────────"
